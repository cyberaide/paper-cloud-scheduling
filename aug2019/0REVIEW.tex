...
Hence we left the definitions in the main text. If however this were
to be an issue that would prevent publication, we certainly can
provide a table instead.}
...
\OK{We heve introduced an introduction section for Section 3
  to describe what the reader is going to read.}
...
\OK{We added a signifia\cant amount more definitions including the
  once pointed out by the reviewer}
...
\OK{In sections 2 and 3, we have mentioned the basic terminology and
  proposed taxonomy. Section 4 presents the literature survey of cloud
  scheduling algorithms and comparison tables are also presented. One
  column Taxonomy classification is included in the table to justify
  the proposed taxonomy. To increase the visibility of the Taxonomy in
the table we highlighted the column that motivated us to include this
classification into the taxonomy. We added a section in the literature
review that makes this clear.}
...
\RESOLUTION{After many discussions, we still believe this section
  including Bare metal provisioning, HPC in the cloud and even Hadoop
is worthwhile to be integrated into the paper to showcase the depth of
the scheduling problems in the cloud. Yes, we have done research in
these areas which precisely motivated us to include this section.
We clarified that this section is important.}
...
\RESOLUTION{All tables have been included as sideways tables. However
  the journal style switches the rotation between odd and even
  numbers. This is likely caused by the journal latex style and could
be fixed by the publication team. However we did not want to modify
the document class ourselves and thus included the sideways tables as
is without attempting to make them display all in one direction.}
...
so instead of pointing to the sections, we just point now to the product/manual}
...
\OK{Years were added to. However as we added new references,
...
\RESOLUTION{A new section was added.}
...
\RESOLUTION{A new section about ML and Deep Learning was added}
...
\RESOLUTION{The paper is included in the updated version}
...
\RESOLUTION{All tables and figures have been cited in the text with

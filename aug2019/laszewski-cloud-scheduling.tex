...
\TODO{for some reason this image is out commented, is content
  elsewhere?. I out-commented it, lets check carefully. The text seems
  also not to match. seems to replicate what we have in Fig. 7.}

\TODO{In the previous submitted paper, the same text was there. Fig
  5 was missing. So in the revised version, fig 5 is included.}
...
scheduling decisions in Cloud computing environment.

\TODO{THIS DOS NOT FOLLOW OUR PAPER STRUCTURE WHERE WE FOCUS ON
  CONTENT AND NOT PEOPLE}

\TODO{Content was added for reference so that it could be changed as per the paper style}

Mingxi Cheng et al.~\cite{cheng2018drl} have proposed two stage
resource provisioning and/or task scheduling processor using deep
reinforcement learning approach. The main intent of the processor is
to generate best long term decisions by observing user request
patterns and realistic electric price. The proposed algorithm helps to
minimize the energy cost efficiency and reduce run-time too.

Hongzi Mao et al. ~\cite{mao2018learning} presented reinforcement
learning based scheduler, Decima, for scheduling complex data
processing jobs. Authors have evaluated Decima on a real Spark cluster
testbed and in simulations with a production workload.The main intent
of scheduler is to make low-latency scheduling decisions.

Qingchen Zhang et al. ~\cite{zhang2017energy} proposed a hybrid
dynamic voltage and frequency scaling (DVFS) scheduling algorithm by
using deep Q-learning model. Their main intent to reduce the energy
consumption.


Yuandou Wang et al. ~\cite{wang2019multi} designed a markov model that
aims to consider number of workflow applications and VMs as state
input and maximum completion time and cost as rewards. They have
applied deep Q network model to schedule the workflow application on
IaaS clouds.

Enda Barret et al. ~\cite{barrett2013applying} proposed a novel
parallel Q learning approach that help to guide the optimal decision
in the process of resource allocation at the time of scheduling. They
have also used markov decision process to handle the uncertainty.  A
comparison of artificial intelligence based scheduling algorithm is
done in~\ref{T:j1}.

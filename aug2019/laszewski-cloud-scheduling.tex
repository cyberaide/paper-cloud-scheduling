...
Service frameworks. It justifies \color{red}
...
\item A systematic survey of cloud scheduling approaches with analysis of their pros and cons.

\item A layered and phased scheduling model that allows to
separate concerns between infrastructure, platform,software, and function as a Service is presented. Scheduling Taxonomy for clouds is also presented.

\item A comprehensive investigation has been conducted to study various existing scheduling techniques in cloud resource management systems accomplished by in-depth learning of resource scheduling algorithms.

\item Analysis of challenges ahead and potential future research directions in the area of Big data processing jobs, analytic as a service in the cloud are presented.
...
\item[Schedulers:] Schedulers are processes that decide which task and
  ...
  \item[Task scheduling:] The process to allocate an ingredient resource to particular task is called task scheduling. The main intent of the task scheduling is enhance the utilization of resources and reduce the task completion time. 
  
  
\item[Application scheduling:]
  
  \item[Job scheduling:] The main intent of the job scheduling is to provide mapping of the job on the particular resource under the fulfilling the requirements of resource in cloud environment. 
\end{description}
\color{black}
...
\TODO{for some reason this image is out commented, is content elsewhere?. I outcommented it, lets check carefully. The text seems also not to match. seems to replicate what we have in Fig. 7.}
...
Figure~\ref{F:graph-challenges} while enhancing the traditional scheduling
...
\color{red}
 Deep Reinforcement Learning (DeepRL) based approaches can automatically learn to schedule more efficiently and can also adapt to any system changes. AI  based approached can be used to handle the scheduling decisions in Cloud computing environment. 

Mingxi Cheng et al.~\cite{cheng2018drl} have proposed two stage resource provisioning and/or task scheduling processor using deep reinforcement  learning approach. The main intent of the processor is to generate best long term decisions by observing user request patterns and realistic electric price. The proposed algorithm helps to minimize the energy cost efficiency and reduce run-time too.

Hongzi Mao et al. ~\cite{mao2018learning} presented reinforcement learning based scheduler, Decima, for scheduling complex data processing jobs. Authors have evaluated Decima on a real Spark cluster testbed and in simulations with a production workload.The main intent of scheduler is to make low-latency scheduling decisions. 

Qingchen Zhang et al. ~\cite{zhang2017energy} proposed  a hybrid dynamic voltage and frequency scaling (DVFS) scheduling algorithm by using deep Q-learning model. Their main intent to reduce the energy consumption. 


Yuandou Wang et al. ~\cite{wang2019multi} designed a markov model that aims to consider number of workflow applications and VMs as state input and maximum completion time and cost as rewards. They have applied deep Q network model to schedule the workflow application on IaaS clouds.

Enda Barret et al. ~\cite{barrett2013applying} proposed a novel parallel Q learning approach that help to guide the optimal decision in the process of resource allocation at the time of scheduling. They have also used markov decision process to handle the uncertainty. 
A comparison of artificial intelligence based scheduling
algorithm is done in~\ref{T:j1}.
...
\item[Cloud Bursting of On Premise HPC tasks.] The HPC systems are
  ...
  \color{red}
  Table ~\refl{T:l} present the comparison of the different batch resource management systems. 
...
\item[Scheduling Challenges Arising form use of Containers.] By using virtualization technologies such as virtual machines, they
...
\item[Challenges in Function as a Service.] The {\em Function as a Service} model allows the developers to build and execute their programs through a combination of functions, that limit resource requirements. Functions are uploaded to FaaS supporting infrastructure and services and triggered by events. Due to the resource limitations they provide significant information for the
